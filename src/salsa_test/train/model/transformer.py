...
class Gate(nn.Module):
    def __init__(self, dimension, scalar, dropout):
        super().__init__()
        self.dropout = dropout
...
class TransformerLayer(nn.Module):
    def __init__(self, params, is_encoder, gated=False):
        ...
        if self.is_decoder:
            self.n_cross_heads = params.n_cross_heads
    ...
    def forward(
        self, x, attn_mask, src_mask, src_enc, use_cache=False, cache=None, loop_count=1
    ):
        tensor = x
        for i in range(loop_count):
            ...
            attn = self.self_attention(
                tensor, attn_mask, use_cache=use_cache, first_loop=(i == 0)
            )
...
class AdaptiveHalt(nn.Module):
    def __init__(self, params, is_encoder, gated):
        ...
        self.ponder_coupling = params.act_ponder_coupling
        self.ponder_penalty = 0
        self.layer = TransformerLayer(params, is_encoder, gated)

    def forward(
        self, input, attn_mask, src_mask, src_enc, use_cache, cache, loop_count
    ):
        bs = input.size(0)
        slen = input.size(1)
        shape = (bs, slen)
        ...
        for i in range(self.max_loops):
            ...
            # update state
            input = self.layer.forward(
                input, attn_mask, src_mask, src_enc, use_cache, cache, loop_count
            )
            # weighted final state
            update_weights = torch.unsqueeze(
        ...
        remainders += torch.less(halting_probability, 1.0).float() * (
            1 - halting_probability
        )
    ...
    def generate_beam(
        ...
        while cur_len < max_len:
            ...
            if self.apex:
                ...
                tensor = tensor.data[
                    -1, :, :
                ]  # .to(self.dtype)  # (bs * beam_size, dim)
...
class RegTransformer(TransformerModel):
    def __init__(self, params, id2word, is_encoder, with_output):
        super().__init__(params, id2word, is_encoder, with_output)
    ...
    def predict(self, tensor, y, get_scores, weighted=False):
        """
        Given the last hidden state, compute word scores and/or the loss.
            `pred_mask` is a ByteTensor of shape (slen, bs), filled with 1 when
                we need to predict a word
            ...
            `get_scores` is a boolean specifying whether we need to return scores
        """
        x = tensor.mean(0).view(-1, self.dim)
        scores = self.proj(x).view(-1)

        loss = torch.mean(1 - torch.cos(scores.float() - y.view(-1)))
        return scores, loss
