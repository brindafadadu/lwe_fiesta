...
def get_optimizer(parameters, s, params=None):
    """
    ...
    if method == "adadelta":
    ...
    elif method == "adamw_warmup":
        assert params is not None, "Provide params"
        betas = (
            optim_params.get("beta1", 0.9),
            optim_params.get("beta2", 0.999),
        )
        ...
        sch = ConstantWithWarmup(op, warmup_steps=optim_params["warmup_updates"])
        return op, sch
    ...
    return optim_fn(parameters, **optim_params), None
